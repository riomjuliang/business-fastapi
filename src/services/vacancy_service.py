...
class VacancyService:
    ...
    def create_vacancy(self, vacancy: VacancyDto):
        existing_company = company_repository.get_company_by_id(vacancy.company_id)
        if not existing_company:
            raise HTTPException(status_code=404, detail="Company with id " + str(vacancy.company_id) + " not exists")

        existing_vacancy = vacancy_repository.get_vacancy_by_id(vacancy.vacancy_id)
        if existing_vacancy:
    ...
    def get_vacancies(self):
        return vacancy_repository.get_all()
    ...
    def update_vacancy(self, vacancy_id: int, vacancy: VacancyDto):
        vacancy_to_update = vacancy_repository.get_vacancy_by_id(vacancy_id)
    ...
    def delete_vacancy(self, vacancy_id: int):
        vacancy_to_delete = vacancy_repository.get_vacancy_by_id(vacancy_id)
        if vacancy_to_delete is None:
            raise HTTPException(status_code=404, detail="Vacancy with vacancy id " + str(vacancy_id) + " not found")
        else:
            return vacancy_repository.delete(vacancy_to_delete)

    def get_vacancies_by_company_id(self, company_id):
        vacancies = vacancy_repository.get_all_by_company_id(company_id)
